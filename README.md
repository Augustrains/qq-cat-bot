...
基于 [NoneBot2](https://github.com/nonebot/nonebot2) + QQ 官方 API + [LangGraph](https://github.com/langchain-ai/langgraph) ReAct agent 的多后端 AI 聊天机器人。支持 tool calling、语义检索、文字/语音双模输出，配备自进化记忆系统。

## 架构

```
QQ 消息 → WebSocket → NoneBot2(:8085) ──┬── LangGraph ReAct agent
                                         │     ├── agent_node (DeepSeek, 流式)
                                         │     ├── tools_node (6 个 MCP 风格工具)
                                         │     └── FAISS + SBERT 语义检索 (Top-K 工具选择)
                                         ├── Claude API (预留)
                                         └── Qwen3-TTS → dashscope.aliyuncs.com

nginx :443 反向代理为 Webhook 备用通道（暂未使用）

定时管道:
cron(2:07 AM) → analyze_history.py → SQLite(24h 消息) → DeepSeek → memory.md + profile.md
```

## 功能

- **ReAct Agent** — LangGraph StateGraph + ToolNode，LLM 自主判断调用工具或直接回复
- **Tool Calling** — 微博搜索、模型切换、输出模式切换、状态查询、清除记忆、重载人设，6 个可插拔工具
- **语义检索** — FAISS + Sentence-BERT (BGE-small-zh) 按查询相关性动态选择 Top-K 工具，控制上下文长度
- **自然对话** — 无需命令前缀，直接发消息即可聊天；精确 slash 命令 (`/clear`、`/status` 等) 绕过 LLM 直接执行
- **多后端支持** — DeepSeek (fast/pro 自动升级) + Claude 预留，运行时通过命令切换
- **流式输出** — token 级流式，文字模式逐句发送，语音模式完整收集后 TTS 合成发送
...
- **新旧架构切换** — `.env` 中 `LANGGRAPH_ENABLED=true/false`，false 时退回旧 if/elif 路由
...
- **Python 3.11+** (LangGraph `get_stream_writer()` 依赖 asyncio contextvar 跨任务传播，Python 3.10 不支持)
...
| `LANGGRAPH_ENABLED` | 是否启用 LangGraph 新架构 (`true`/`false`) | 是 |
...
├── plugins/
│   ├── __init__.py
│   ├── auto_chat.py               # 旧架构（LANGGRAPH_ENABLED=false 时生效）
│   ├── chat_history.py            # SQLite 存储模块，asyncio.to_thread 异步写入
│   ├── weibo_search.py            # 微博搜索插件，访客认证 + 热度排序
│   └── langgraph_bot/             # ★ LangGraph ReAct agent 新架构
│       ├── __init__.py            # NoneBot2 入口 + slash 预处理 + retriever 初始化
│       ├── agent.py               # StateGraph + sync agent_node + 条件边
│       ├── state.py               # AgentState TypedDict
│       ├── models.py              # ChatOpenAI 工厂（DeepSeek）
│       ├── context.py             # mtime 人设加载
│       ├── sessions.py            # 会话管理（backend/output_mode/model_override）
│       ├── streaming.py           # text/voice 输出 + TTS
│       ├── tool_gate.py           # 上下文溢出保护（工具数 > 阈值时关键词过滤）
│       ├── retriever/
│       │   ├── __init__.py        # search_tools() + init_retriever()
│       │   ├── encoder.py         # Sentence-BERT 编码（绕过 ST 5.x 递归，直调底层 forward）
│       │   ├── index_store.py     # FAISS IndexFlatIP 管理
│       │   └── tool_meta.py       # SQLite tools 表 CRUD
│       └── tools/
│           ├── __init__.py        # 注册表 + TOOL_META 元数据
│           ├── weibo.py           # search_weibo
│           ├── mode_switch.py     # switch_model / switch_output
│           ├── status.py          # show_bot_status
│           ├── clear.py           # clear_history
│           └── reload_char.py     # reload_character
├── characters/
│   └── default/
│       ├── system.md              # 基础人设 — 黑色布偶猫人格（手动编辑）
│       ├── memory.md              # 经验积累（分析脚本每日自动追加）
│       ├── profile.md             # 用户画像（分析脚本每日自动覆写）
│       ├── analysis_prompt.md     # 分析提示词模板
│       └── suggestions.txt        # 自动生成的改进建议
├── scripts/
...
机器人的人设由 `characters/default/` 下的文件定义：
...
国内服务器访问 Anthropic API (Claude) 和 HuggingFace 模型下载需要代理。DeepSeek 和阿里云百炼 TTS 均为国内服务，**无需代理**。
...
> `HF_HUB_OFFLINE=1` 防止 Sentence-BERT 启动时尝试连网检查模型更新（模型已缓存到本地）。
...
如启用 LangGraph 架构，首次启动前需下载语义检索模型：

```bash
# 通过代理下载（模型 ~100MB）:
HTTP_PROXY=http://127.0.0.1:7897 HTTPS_PROXY=http://127.0.0.1:7897 \
  HF_ENDPOINT=https://hf-mirror.com \
  /home/ubuntu/robot/venv/bin/python -c \
  "from sentence_transformers import SentenceTransformer; SentenceTransformer('BAAI/bge-small-zh-v1.5'); print('OK')"
```

下载后模型缓存在 `~/.cache/huggingface/hub/`，后续启动无需代理。如模型未下载，bot 启动时会自动 fallback 为全部工具直给 LLM（功能正常，仅不做语义过滤）。
...
```bash
mkdir -p /home/ubuntu/robot/logs
crontab -e
# 添加以下行:
...
│                     │                             │
│  qqbot.service       ├── LangGraph ReAct agent    │
│  (systemd)           │   ├── DeepSeek (国内直连)  │
│                      │   ├── Claude  (→ mihomo)   │
│                      │   ├── TTS    (国内直连)    │
│                      │   └── FAISS + SBERT        │
│                                                   │
...
- **ReAct Agent**: LangGraph StateGraph + ToolNode，sync agent_node + `get_stream_writer()` 实现 token 级流式。工具通过 FAISS 语义检索动态选择，控制上下文长度
- **正交设计**: LLM 后端 (DeepSeek/Claude) 和输出模式 (文字/语音) 独立抽象，新后端/新模式只需实现对应接口
- **新旧切换**: `.env` `LANGGRAPH_ENABLED` 控制新旧架构，false 时退回 `auto_chat.py` 旧路由
- **配置集中**: 所有配置在 `.env`，`bot.py` 启动时 `load_dotenv()` 注入 `os.environ`，systemd 服务文件不含密钥
- **会话隔离**: 每用户独立 sessions dict，不同后端间对话历史独立
- **并发安全**: SQLite WAL 模式 + `busy_timeout=5000`，多线程写入不冲突，跨进程读写不互斥。asyncio 单线程安全 — 工具 module-level 上下文无需锁
...
- **分析管道混用多用户**: 当前分析脚本将所有用户的消息混合分析，profile.md 画像不区分用户。后续需按 user_id 拆分独立画像
- **sentence_transformers 5.5.1 递归 bug**: `encode()` 内部 `to(device)` / `eval()` 触发模块树无限递归。已绕过：`encoder.py` 直调 tokenizer + model forward。升级依赖后需验证是否修复
- **InMemorySaver**: bot 重启丢失对话历史（SQLite 持久化不受影响）
